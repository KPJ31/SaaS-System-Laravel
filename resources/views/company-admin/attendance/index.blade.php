...
<div class="stat-grid">
    @foreach([
        ...
        ['Checked In', $stats['checked_in'], 'fa-user-check', 'green'],
        ['Late', $stats['late'], 'fa-clock-rotate-left', 'yellow'],
        ['Half Day', $stats['half_day'], 'fa-circle-half-stroke', 'blue'],
        ['Absent', $stats['absent'], 'fa-user-xmark', 'yellow'],
...
<section class="content-card mb-3">
    <form class="row g-3 align-items-end" method="GET" action="{{ route('company-admin.attendance.index') }}">
        <div class="col-md-2"><label class="form-label">Date</label><input class="form-control" type="date" name="date" value="{{ request('date', $date) }}"></div>
            ...
            <tbody>
                @forelse($attendances as $attendance)
                    <tr>
                        ...
                        <td>
                            <form method="POST" action="{{ route('company-admin.attendance.update', $attendance) }}" class="d-grid gap-2" data-loading-form>
                                @csrf @method('PATCH')
                                ...
                                <input class="form-control form-control-sm" name="note" value="{{ $attendance->note }}" placeholder="Note">
                                <input class="form-control form-control-sm" name="correction_reason" placeholder="Correction reason" required>
                                <button class="btn btn-sm btn-outline-primary" type="submit"><i class="fa-solid fa-check"></i>Correct</button>
                            </form>
                        </td>
                    </tr>
                @empty
