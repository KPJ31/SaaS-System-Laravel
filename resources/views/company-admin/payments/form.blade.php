...
<form class="content-card" method="POST" action="{{ $payment->exists ? route('company-admin.payments.update', $payment) : route('company-admin.payments.store') }}" data-loading-form>
    ...
    <div class="row g-3">
        <div class="col-md-4"><label class="form-label">Invoice</label><select class="form-select" name="invoice_id"><option value="">No invoice link</option>@foreach($invoices as $invoice)<option value="{{ $invoice->id }}" @selected(old('invoice_id', $payment->invoice_id)==$invoice->id)>{{ $invoice->invoice_number }} - {{ $invoice->client?->name }} - {{ $currency }} {{ number_format($invoice->balance_amount, 2) }} due</option>@endforeach</select></div>
        ...
        <div class="col-md-4"><label class="form-label">Project</label><select class="form-select" name="project_id"><option value="">Choose project</option>@foreach($projects as $project)<option value="{{ $project->id }}" data-client-id="{{ $project->client_id }}" @selected(old('project_id', $payment->project_id)==$project->id)>{{ $project->name }}</option>@endforeach</select></div>
