...
<div class="content-grid">
    <section class="content-card"><h2>Payment</h2><dl class="detail-list mt-3"><dt>Status</dt><dd>@include('partials.status-badge', ['status' => $payment->status])</dd><dt>Amount</dt><dd>{{ $currency }} {{ number_format($payment->amount, 2) }}</dd><dt>Invoice</dt><dd>@if($payment->invoice)<a href="{{ route('company-admin.invoices.show', $payment->invoice) }}">{{ $payment->invoice->invoice_number }}</a>@else - @endif</dd><dt>Client</dt><dd>{{ $payment->client?->name ?? '-' }}</dd><dt>Project</dt><dd>{{ $payment->project?->name ?? '-' }}</dd><dt>Method</dt><dd>{{ $payment->method }}</dd><dt>Paid Date</dt><dd>{{ $payment->paid_at?->format('Y-m-d') ?? '-' }}</dd><dt>Verified By</dt><dd>{{ $payment->verifier?->name ?? '-' }}</dd><dt>Verification Note</dt><dd>{{ $payment->verification_note ?? '-' }}</dd></dl></section>
