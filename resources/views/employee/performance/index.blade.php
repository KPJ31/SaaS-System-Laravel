...
<div class="content-grid"><section class="content-card"><h2>Score Breakdown</h2><dl class="detail-list mt-3"><dt>Completed task rate</dt><dd>{{ $completedRate }}%</dd><dt>Time efficiency</dt><dd>{{ $timeEfficiency === null ? 'Not enough data' : $timeEfficiency.'%' }}</dd><dt>Work consistency</dt><dd>{{ $consistency }}%</dd></dl></section><section class="content-card"><h2>Monthly Completed Tasks</h2><div class="dashboard-chart-wrapper chart-small"><canvas id="employeePerformanceChart" data-chart="employeePerformance" role="img" aria-label="Monthly completed tasks performance chart"></canvas></div></section></div>
