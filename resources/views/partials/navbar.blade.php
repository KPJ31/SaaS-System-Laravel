        ...
        <div class="dropdown">
            ...
            <div class="dropdown-menu dropdown-menu-end notification-menu">
                <div class="dropdown-header d-flex justify-content-between align-items-center">
                    ...
                    @php
                        $notificationPrefix = auth()->user()->role === 'super_admin' ? 'super-admin' : (auth()->user()->role === 'company_admin' ? 'company-admin' : 'employee');
                    @endphp
                    @if(auth()->user()->unreadNotifications()->count() && Route::has($notificationPrefix.'.notifications.read-all'))
                        <form method="POST" action="{{ route($notificationPrefix.'.notifications.read-all') }}">
                            @csrf
                ...
                @forelse(auth()->user()->notifications()->latest()->take(5)->get() as $notification)
                    <a class="dropdown-item notification-item {{ $notification->read_at ? '' : 'is-unread' }}" href="{{ route($notificationPrefix.'.notifications.index') }}">
                ...
                <a class="dropdown-item text-center" href="{{ route($notificationPrefix.'.notifications.index') }}">View all notifications</a>
            </div>
        </div>
        <div class="dropdown">
